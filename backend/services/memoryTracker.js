  ...
  return new Promise((resolve) => {
    exec('docker stats --no-stream --format "{{.Name}}|{{.MemUsage}}|{{.MemPerc}}"', (err, stdout) => {
      ...
      try {
        ...
        resolve(containers.filter(c => c.name));
      } catch {
        resolve([]);
      }
    });
...
// Anomali tespiti
async function checkAnomalies(currentServices) {
...
function getMemoryStats(hours = 1) {
...
module.exports = { startTracker, stopTracker, getMemoryStats, getCurrentServices };
